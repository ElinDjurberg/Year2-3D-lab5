...
int main(int argc, char *argv[]) {
    ...
    // Sun
    Texture sunTexture;
    /* behövs ingen ny TriangleSoup och ingen ny Shader eftersom samma objekt(triangelsoup) kan användas till de andra objekten.
    i detta fall är alla sfär och ska ha samma ljussättning(shader)
    */
    ...
    TriangleSoup pyramid;
    //pyramid.readOBJ("meshes/pyramid.obj");
	...
	pyramidTexture.createTexture("textures/pyramid.tga");
    ...
    while(!glfwWindowShouldClose(window))
    {
		...
		glUseProgram( earthShader.programID );//Ska vi aktivera för sol och måne också??
        ...
        MVstack.push(); // Save the initial, untouched matrix
            ...
            MVstack.push(); // Save the current matrix on the stack
                ...
                earthSphere.render();//Ska man ändra denna??????

            MVstack.pop(); // Restore the matrix we saved above

        //Pyramid
            MVstack.push();
                MVstack.translate(2.0f, 0.1f, 0.0f);

                // Update the transformation matrix in the shader
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
                // Render the geometry to draw the sun
                glBindTexture(GL_TEXTURE_2D, pyramidTexture.texID);
                pyramid.render();//Ska vi ändra denna???
            MVstack.pop();
            //Pyramid slutar
           ...
           MVstack.translate(1.0f, 0.0f, 0.0f); // Flyttar ut i x led och höjer lite i y led i förhållande till jorden
            ...
            // Moon EGEN
            MVstack.rotY(0.7f*time); // Rotation, omloppsbanan, långsammare i förhållande till jorden
            MVstack.translate(0.3f, 0.1f, 0.0f); // Flyttar ut i x led och höjer lite i y led i förhållande till jorden


                MVstack.rotY(5.0f*time); // Snurrar runt sin egen axel, hastighet i förhållande till jorden
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z   DENNA GÖR SÅ ATT TEXTUREN HAMNAR PÅ RÄTT AXEL. eX NORDPOLEN HAMNAR I AFRIKA ANNARS
            ...
            earthSphere.render();//Ska vi ändra denna???
